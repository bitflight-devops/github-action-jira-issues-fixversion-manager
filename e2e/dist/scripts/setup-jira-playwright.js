...
function generateLicense(serverId) {
    ...
    try {
        const output = (0, node_child_process_1.execSync)(cmd, { encoding: 'utf-8', timeout: 60000 });
        ...
        console.error(`  Error generating license: ${e.message}`);
        return null;
    }
}
/**
 ...
 */
function waitForPluginRestart(sinceTimestamp, timeoutMs) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        // Fail fast checks
        if (logs.match(/OutOfMemoryError/i))
            throw new Error('Jira crashed: OutOfMemoryError');
        if (logs.match(/FATAL/i))
            throw new Error('Jira crashed: FATAL error in logs');
        // Optional: Log progress
        if (logs.match(/Starting the JIRA Plugin System/i) && elapsed % 5 === 0) {
            console.log(`  [${elapsed}s] ⏳ Restart in progress...`);
        }
        (0, node_child_process_1.execSync)('sleep 2');
    }
    return false;
}
// =============================================================================
...
async function main() {
    ...
    for (let i = 0; i < 40; i++) {
        ...
        (0, node_child_process_1.execSync)('sleep 3');
    }
    if (!bootReady) {
    ...
    try {
        ...
        if (attempts >= 60)
            throw new Error('Timeout waiting for Web UI');
        ...
        if (await licenseBox.isVisible()) {
            ...
            const serverIdMatch = content.match(/Server ID[:\s]*<[^>]*>([A-Z0-9-]+)/i) ||
            ...
            if (serverIdMatch?.[1]) {
                ...
                if (!restarted)
                    throw new Error('Timed out waiting for Plugin System Restart');
        ...
        for (let i = 0; i < 15; i++) {
            ...
            const nextButtons = page.locator('button:has-text("Next"):visible, button:has-text("Finish"):visible, button:has-text("Continue"):visible, input[type="submit"]:visible');
