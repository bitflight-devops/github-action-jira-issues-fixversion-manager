...
function exec(cmd, timeout = 30000) {
    try {
        return (0, node_child_process_1.execSync)(cmd, { encoding: 'utf-8', timeout, stdio: ['pipe', 'pipe', 'pipe'] });
    }
    catch (error) {
        const err = error;
 ...
 */
function execQuiet(cmd, timeout = 30000) {
    try {
        return (0, node_child_process_1.execSync)(cmd, { encoding: 'utf-8', timeout, stdio: ['pipe', 'pipe', 'pipe'] });
    }
    catch {
...
async function waitForLogPattern(pattern, timeoutMs) {
    return new Promise((resolve) => {
        ...
        const dockerLogs = (0, node_child_process_1.spawn)('docker', ['logs', '-f', '--since', '1s', CONTAINER_NAME], {
 ...
 */
function getDockerLogs(lines = 50) {
 ...
 */
async function getServerId(baseUrl) {
    // Try REST API first
    try {
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
        try {
            ...
            console.log(`  Attempt ${attempt}/10: ${error.message}`);
        }
        await sleep(5000);
    }
    return null;
 ...
 */
function generateLicense(serverId) {
    try {
        const output = exec(`docker exec ${CONTAINER_NAME} java -jar /var/agent/atlassian-agent.jar ` +
            `-d -p jira -m test@example.com -n test@example.com -o TestOrg -s ${serverId}`, 30000);
 ...
 */
function insertLicenseViaDatabase(license) {
    try {
        ...
        const tableExists = execQuiet(`docker exec jira-e2e-mysql mysql -uroot -p123456 -N -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='jira' AND table_name='productlicense'" 2>/dev/null`);
        if (!tableExists.trim() || tableExists.trim() === '0') {
            ...
            const tables = execQuiet(`docker exec jira-e2e-mysql mysql -uroot -p123456 -N -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='jira'" 2>/dev/null`);
            console.log(`  Tables in jira database: ${tables.trim() || '0'}`);
            return false;
        }
        // Check if the table has any entries
        const checkResult = execQuiet(`docker exec jira-e2e-mysql mysql -uroot -p123456 -N -e "SELECT COUNT(*) FROM jira.productlicense" 2>/dev/null`);
        const existingCount = Number.parseInt(checkResult.trim(), 10);
        if (existingCount > 0) {
            // Update existing license
            console.log('  Updating existing license in database...');
            exec(`docker exec jira-e2e-mysql mysql -uroot -p123456 -e "UPDATE jira.productlicense SET LICENSE='${escapedLicense}' WHERE ID=(SELECT MIN(ID) FROM (SELECT ID FROM jira.productlicense) AS t)" 2>/dev/null`);
        }
        else {
            // Insert new license
            console.log('  Inserting license into database...');
            exec(`docker exec jira-e2e-mysql mysql -uroot -p123456 -e "INSERT INTO jira.productlicense (ID, LICENSE) VALUES (10000, '${escapedLicense}')" 2>/dev/null`);
        }
        // Verify the license was inserted
        const verifyResult = execQuiet(`docker exec jira-e2e-mysql mysql -uroot -p123456 -N -e "SELECT COUNT(*) FROM jira.productlicense WHERE LICENSE IS NOT NULL" 2>/dev/null`);
        ...
        console.log(`  Database license insertion error: ${error.message}`);
        return false;
    }
}
/**
 ...
 */
async function submitLicenseViaHttp(baseUrl, license) {
    ...
    try {
        ...
        console.log(`  License submission error: ${error.message}`);
        return false;
    }
}
/**
 ...
 */
async function submitLicense(baseUrl, license) {
 ...
 */
async function restartJiraContainer() {
    try {
 ...
 */
async function completeSetup(baseUrl, username, password) {
    const steps = [
        {
            name: 'App Properties',
 ...
 */
async function verifyJiraReady(baseUrl, username, password) {
