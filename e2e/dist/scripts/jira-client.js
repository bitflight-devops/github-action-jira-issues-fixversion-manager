 ...
 */
function extractJiraErrorDetails(error) {
    const err = error;
...
class JiraE2EClient {
     ...
     */
    async createProjectWithLead(basePayload, isCloud, leadIdentifier) {
        if (isCloud) {
        ...
        if (!response.ok) {
            ...
            const fieldErrors = errorData.errors
                ? Object.entries(errorData.errors)
                    ...
                    .join(', ')
                : '';
            throw new Error(`Request failed with status code ${response.status}${fieldErrors ? ` | Fields: ${fieldErrors}` : ''}${errorMsg ? ` | ${errorMsg}` : ''}`);
        }
    ...
    async ensureProject(key, name) {
        try {
            ...
            try {
                ...
                const maskedId = identifier.length > MIN_ID_LENGTH_FOR_MASKING
                    ? `${identifier.slice(0, 4)}...${identifier.slice(-4)}`
                    : identifier;
                console.log(`  Detected ${userInfo.isCloud ? 'Cloud' : 'Data Center'}, using lead: ${maskedId}`);
            }
            catch (userError) {
                console.error(`  Failed to get current user info: ${userError.message}`);
                throw new Error(`Cannot create project: Unable to determine user info - ${userError.message}`);
            }
            ...
            try {
                ...
                try {
                    ...
                    throw new Error(`Failed to create project ${key}: Software type failed (${errorMsg}), Business type failed (${businessErrorMsg})`);
                }
            }
        }
    }
    ...
    async ensureIssue(projectKey, summary, issueType = 'Task', fixVersions) {
        ...
        const searchResult = await this.searchIssues(`project = ${projectKey} AND summary ~ "${summary}" ORDER BY created DESC`, ['summary', 'fixVersions', 'status']);
     ...
     */
    async configureScreensForFixVersions() {
        try {
            ...
            for (const screen of screens) {
                ...
                try {
                    ...
                    console.log(`  Could not get tabs for screen ${screen.name}: ${tabError.message}`);
                }
            }
        }
        catch (error) {
