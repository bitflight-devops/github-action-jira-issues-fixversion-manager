...
This directory contains end-to-end (E2E) tests for the Jira FixVersion Manager GitHub Action using a dockerized Jira Data Center instance.
...
1. **Start the Jira stack:**
   ...
   This starts MySQL and Jira Software containers.
...
- `mysql-data-snapshot.tar.gz` - Database data
...
│   └── jira-dbconfig.xml         # Jira database configuration for MySQL
├── scripts/
...
│   ├── setup-jira-playwright.ts  # Headless browser setup automation
...
│   ├── mysql-data-snapshot.tar.gz
│   └── snapshot-metadata.json
└── tests/
    └── fixversion.e2e.test.ts    # FixVersion management E2E tests
```
...
The E2E tests cover FixVersion management functionality:

### FixVersion Management Tests (`fixversion.e2e.test.ts`)

- Project and version setup verification
...
- Edge cases (special characters, empty versions, version listing)
...
3. Use Vitest's standard test structure
